    ...
    return (
        <View>
            ...
            <TextInput
                ...
                onFocus={input.onFocus}
                maxLength={12}
                placeholder='Digite seu número incluindo DDD...'
                value={input.value}
                underlineColorAndroid={colors.white}
                style={{ color: colors.white }}
...
class PhoneAuth extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            mensagem: '',
            contactPhoneNumber: this.props.navigation.state.params.phoneNumber,
        };
    ...
    componentDidMount() {
        ...
        this.unsubscribe = firebase.auth().onAuthStateChanged((user) => {
            if (user) {
                ...
                this.props.navigation.navigate('Buscou')
            } else {
                // User has been signed out, reset the state
                this.props.resetUserStatus;
            }
    ...
    _submit = values => {
        ...
        return this.props.getPhoneAuthCode(values.telefone, values.nome);
    ...
    _submit2 = values => {
        ...
        if (confirmResult && codeInputed.length) {
            store.dispatch(ActionCreators.confirmCodeFromOutSide(confirmResult, codeInputed, mensagem, name, this.state.contactPhoneNumber));
        }
    }
    ...
    render() {
        const { user, confirmResult, } = this.props;
        const { userAuthorized, codeInput } = this.state;
...
const mapDispatchToProps = (dispatch) => {
    return {
        ...
        sendingMsg: () => { dispatch(ActionCreators.sendingMsgPrinc()) },
